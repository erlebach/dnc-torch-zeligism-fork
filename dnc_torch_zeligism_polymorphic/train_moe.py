...
from dnc_torch_zeligism_polymorphic.configuration import controller_config, memory_config, training_config
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Train a Mixture of Experts DNC on the Repeat-Copy task')
    parser.add_argument('--num_experts', type=int, default=4, help='Number of expert memory units')
    parser.add_argument('--top_k', type=int, default=2, help='Number of experts to route each input to')
    parser.add_argument('--capacity_factor', type=float, default=1.5, help='Factor to determine expert capacity')
    parser.add_argument('--router_jitter', type=float, default=0.01, help='Noise added to router logits during training')
    ...
    parser.add_argument('--learning_rate', type=float, default=training_config['learning_rate'], help='Learning rate')
    parser.add_argument('--num_examples', type=int, default=training_config['num_examples'], help='Number of examples to train on')
    parser.add_argument('--checkpoint', type=int, default=training_config['checkpoint'], help='Checkpoint interval')
    parser.add_argument('--seed', type=int, default=training_config['random_seed'], help='Random seed')
    parser.add_argument('--log_dir', type=str, default='logs/moe', help='Directory for TensorBoard logs')
    parser.add_argument('--load_balancing_coef', type=float, default=0.01, help='Coefficient for load balancing loss')
...
def train(model: ParallelControllerMoE, task: RepeatCopy, args: argparse.Namespace) -> None:
    ...
    for i in range(args.num_examples):
        ...
        if (i + 1) % args.checkpoint == 0:
            ...
            print(f"[{i+1}/{args.num_examples}] Task Loss = {task_loss.item():.3f}, Total Loss = {loss.item():.3f}, Examples/sec = {examples_per_second:.2f}")
